...
FACTORS = [
    Factor("capital",    "Paid-up Capital",      9, "₹1Cr–₹25Cr is ideal for SME IPO eligibility"),
    Factor("headroom",   "Capital Headroom",      6, "Higher auth–paid gap supports fresh equity raising"),
    Factor("vintage",    "Company Vintage",       6, "3–12 years is the optimal growth window for IPO"),
    Factor("structure",  "Corporate Structure",   4, "Private Ltd preferred for conversion to Public"),
    Factor("sector",     "Sector Momentum",       8, "Investor appetite and SEBI listing demand by sector"),
    Factor("status",     "Active Status",         5, "Active MCA status required for IPO eligibility"),
    Factor("geography",  "Geography",             3, "Proximity to financial centres improves deal access"),
    Factor("disclosure", "Disclosure Depth",      5, "Email, address, website completeness score"),
    Factor("signal",     "Data Confidence",       4, "Overall MCA record data quality"),
]

# ─── CSS — OLIVE GREEN LIGHT THEME ──────────────────────────
def inject_css() -> None:
    st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@300;400;500;600;700&family=Source+Serif+4:opsz,wght@8..60,600;8..60,700&family=IBM+Plex+Mono:wght@400;500;600&display=swap');
    ...
    [data-testid="stMain"], [data-testid="stAppViewBlockContainer"] {
        ...
        color: #1A2310 !important;
    }
    [data-testid="stSidebar"] {
        background-color: #EDF1E6 !important;
        border-right: 2px solid #B8C9A3 !important;
    }
    [data-testid="stHeader"] {
        background-color: rgba(242,245,238,0.97) !important;
        border-bottom: 1px solid #B8C9A3 !important;
    }

    h1 { font-family: 'Source Serif 4',serif !important; color: #2D4A1E !important; font-size: 1.9rem !important; }
    h2 { font-family: 'Source Serif 4',serif !important; color: #2D4A1E !important; font-size: 1.4rem !important; }
    h3 { font-family: 'Source Serif 4',serif !important; color: #2D4A1E !important; font-size: 1.1rem !important; }

    .stButton > button {
        background: linear-gradient(160deg, #3A6B25 0%, #2D5218 100%) !important;
        ...
        box-shadow: 0 3px 10px rgba(45,82,24,0.2), inset 0 1px 0 rgba(201,168,76,0.3) !important;
        transition: all 0.15s !important;
    }
    .stButton > button:hover {
        background: linear-gradient(160deg, #2D5218 0%, #1E380F 100%) !important;
        box-shadow: 0 5px 16px rgba(45,82,24,0.3), inset 0 1px 0 rgba(201,168,76,0.4) !important;
        transform: translateY(-1px) !important;
    }
    .stButton > button:disabled {
        background: #C5D4B3 !important;
        color: #8A9E7A !important;
        box-shadow: none !important;
        border-color: #B8C9A3 !important;
    }
    .stDownloadButton > button {
        background: #FAFAF0 !important;
        color: #2D5218 !important;
        border: 1.5px solid #C9A84C !important;
        border-radius: 9px !important;
        font-weight: 600 !important;
        font-family: 'IBM Plex Sans', sans-serif !important;
    ...
    .stTextInput input, .stTextArea textarea, .stNumberInput input {
        ...
        border: 1.5px solid #B8C9A3 !important;
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        color: #5A7042 !important;
    }
    .stTabs [aria-selected="true"] {
        background: #2D5218 !important;
        border-color: #C9A84C !important;
        color: #F5F0DC !important;
        font-weight: 700 !important;
        box-shadow: 0 2px 10px rgba(45,82,24,0.25) !important;
    }

    div[data-testid="stMetric"] {
        background: #FAFAF5 !important;
        border: 1px solid #B8C9A3 !important;
    ...
    summary { color: #2D5218 !important; font-weight: 600 !important; }
    .stSuccess { border-radius: 9px !important; }
    .stWarning { border-radius: 9px !important; }
    .stInfo    { border-radius: 9px !important; }
    hr { border-color: #B8C9A3 !important; border-width: 1px !important; }
    [data-testid="stSidebar"] h3 { color: #2D4A1E !important; font-size:0.9rem !important; }
    .stCaption, [data-testid="stCaption"] { color: #6B8055 !important; font-size: 0.82rem !important; }
    div[data-testid="stMarkdownContainer"] p { color: #1A2310 !important; }
    div[data-testid="stMarkdownContainer"] li { color: #1A2310 !important; }
    div[data-testid="stMarkdownContainer"] strong { color: #2D4A1E !important; }
    div[data-testid="stMarkdownContainer"] a { color: #2D5218 !important; }
    </style>
...
def score_one(row: pd.Series, W: dict) -> int:
    ...
    sm = {"capital":cap(),"headroom":headroom(),"vintage":vintage(),"structure":structure(),
          "sector":sector(),"status":status(),"geography":geography(),"disclosure":disclosure(),"signal":signal()}
    tw = sum(w.values())
...
def render_sidebar(base: pd.DataFrame) -> pd.DataFrame:
    with st.sidebar:
        ...
        st.caption("0 = exclude · 10 = highest weight")
        W: dict[str,int] = {}
        for f in FACTORS:
            W[f.key] = int(st.number_input(
                f.label, min_value=0, max_value=10,
                value=int(st.session_state.W.get(f.key,f.weight)),
                step=1, help=f.desc, key=f"w_{f.key}",
            ))
        st.session_state.W = W
        st.caption(f"Total weight: {sum(W.values())}")
        st.markdown(" ")
